...
def iv0(m,z):
    ...
    #     return 1
    # else:
    #     return 4*z*np.sinh(z+1j*m*np.pi/2)/(4*z**2+m**2*np.pi**2)

    ## Square wave
    if m==0:
        return np.cosh(z)
    else:
        return np.sinh(z)*np.sin(m*np.pi/2)/(m*np.pi/2)

iv=np.vectorize(iv0,otypes=[np.complex128])
...
def viscid_mat2d (omega, argsdict):
    ...
    #kp k
    vals00 = rfac0*cfac0 * \
        ((1-argsdict['mu']/(argsdict['rho']*Omega)*kappax*(kappax-kappapx))*Ctilde - (2*argsdict['mu']*kappax*kappapx*C)/(argsdict['rho']*Omega+argsdict['mu']*kappa**2))
    vals01 = -rfac1*cfac0 * \
        (2*argsdict['mu']*kappapx*kappay*C/(argsdict['rho']*Omega+argsdict['mu']*kappa**2)+argsdict['mu']/(argsdict['rho']*Omega)*kappay*(kappax-kappapx)*Ctilde)
    vals02 = -1j*rfac2*cfac0*((argsdict['mu']*(argsdict['rho']/argsdict['mu']*Omega)**0.5*kappax-argsdict['mu']*kappa**2*(kappax-kappapx)/(argsdict['rho']/argsdict['mu']*Omega)**0.5) * (Ctilde-Stilde) + (argsdict['rho']*Omega+argsdict['mu']*kappa**2)*kappapx*S/(2*kappa) + (argsdict['g']*argsdict['rho']**2+kappa**2*(argsdict['rho']*argsdict['sigma'] - 4*argsdict['mu']**2*(argsdict['rho']*Omega/argsdict['mu'])**0.5))*kappapx*C/(2*(argsdict['rho']*Omega+argsdict['mu']*kappa**2)))

    vals10 = -rfac0*cfac1 * \
        (2*argsdict['mu']*kappapy*kappax*C/(argsdict['rho']*Omega+argsdict['mu']*kappa**2)+argsdict['mu']/(argsdict['rho']*Omega)*kappax*(kappay-kappapy)*Ctilde)
    vals11 = rfac1*cfac1 * \
        ((1-argsdict['mu']/(argsdict['rho']*Omega)*kappay*(kappay-kappapy))*Ctilde - (2*argsdict['mu']*kappay*kappapy*C)/(argsdict['rho']*Omega+argsdict['mu']*kappa**2))
    vals12 = -1j*rfac2*cfac0*((argsdict['mu']*(argsdict['rho']/argsdict['mu']*Omega)**0.5*kappay-argsdict['mu']*kappa**2*(kappay-kappapy)/(argsdict['rho']/argsdict['mu']*Omega)**0.5) * (Ctilde-Stilde) + (argsdict['rho']*Omega+argsdict['mu']*kappa**2)*kappapy*S/(2*kappa) + (argsdict['g']*argsdict['rho']**2+kappa**2*(argsdict['rho']*argsdict['sigma'] - 4*argsdict['mu']**2*(argsdict['rho']*Omega/argsdict['mu'])**0.5))*kappapy*C/(2*(argsdict['rho']*Omega+argsdict['mu']*kappa**2)))

    vals20 = 1j*rfac0*cfac2 * \
        (-2*argsdict['mu']*kappax*(kappax*kappapx+kappay*kappapy)/kappa*S/(argsdict['rho']*Omega+argsdict['mu']*kappa**2)+kappapx*Stilde/(argsdict['rho']*Omega/argsdict['mu'])**0.5)
    vals21 = 1j*rfac1*cfac2 * \
        (-2*argsdict['mu']*kappay*(kappax*kappapx+kappay*kappapy)/kappa*S/(argsdict['rho']*Omega+argsdict['mu']*kappa**2)+kappapy*Stilde/(argsdict['rho']*Omega/argsdict['mu'])**0.5)
    vals22 = rfac2*cfac2*(kappax*kappapx+kappay*kappapy)*(-argsdict['mu']*(Ctilde-Stilde)+(argsdict['rho']*Omega + argsdict['mu']*kappa**2)*C/(2*kappa**2)+(argsdict['g']*argsdict['rho']**2+kappa**2*(argsdict['rho']*argsdict['sigma']-4*argsdict['mu']**2*(argsdict['rho'] * Omega/argsdict['mu'])**0.5))*S/(2*kappa*(argsdict['rho']*Omega+argsdict['mu']*kappa**2)))
    ...
    for lind in range(2*argsdict['Nt']+1):
        E[0,lind,:,:,0,lind,:,:]=vals00[0,0,:,:,0,lind,:,:]
        E[0,lind,:,:,1,lind,:,:]=vals01[0,0,:,:,0,lind,:,:]
        E[0,lind,:,:,2,lind,:,:]=vals02[0,0,:,:,0,lind,:,:]
        E[1,lind,:,:,0,lind,:,:]=vals10[0,0,:,:,0,lind,:,:]
        E[1,lind,:,:,1,lind,:,:]=vals11[0,0,:,:,0,lind,:,:]
        E[1,lind,:,:,2,lind,:,:]=vals12[0,0,:,:,0,lind,:,:]
        E[2,lind,:,:,0,lind,:,:]=vals20[0,0,:,:,0,lind,:,:]
        E[2,lind,:,:,1,lind,:,:]=vals21[0,0,:,:,0,lind,:,:]
        E[2,lind,:,:,2,lind,:,:]=vals22[0,0,:,:,0,lind,:,:]

    vals20 = -1j*(rfac2*cfac0*argsdict['rho']**2*kappapx*C/(4*(argsdict['rho']*Omega+argsdict['mu']*kappa**2)))
    vals21 = -1j*(rfac2*cfac1*argsdict['rho']**2*kappapy*C/(4*(argsdict['rho']*Omega + argsdict['mu']*kappa**2)))
    vals22 = (rfac2*cfac2*argsdict['rho']**2*(kappax*kappapx+kappay*kappapy)*S/(4*kappa*(argsdict['rho']*Omega + argsdict['mu']*kappa**2)))
    ...
    for lind in range(2*argsdict['Nt']):
        F[0,lind,:,:,2,lind+1,:,:]=argsdict['g']*vals20[0,0,:,:,0,lind,:,:]
        F[1,lind,:,:,2,lind+1,:,:]=argsdict['g']*vals21[0,0,:,:,0,lind,:,:]
        F[2,lind,:,:,2,lind+1,:,:]=argsdict['g']*vals22[0,0,:,:,0,lind,:,:]
        F[0,lind+1,:,:,2,lind,:,:]=argsdict['g']*vals20[0,0,:,:,0,1+lind,:,:]
        F[1,lind+1,:,:,2,lind,:,:]=argsdict['g']*vals21[0,0,:,:,0,1+lind,:,:]
        F[2,lind+1,:,:,2,lind,:,:]=argsdict['g']*vals22[0,0,:,:,0,1+lind,:,:]
...
def rayleigh_mat(omega_0, v0, w0, mat, argsdict, mat2=None):
    ...
    for n in range(argsdict['itmax']):
        ...
        verr=np.linalg.norm(dv,ord=2)/(1+np.linalg.norm(vn,ord=2))
        werr=np.linalg.norm(dw,ord=2)/(1+np.linalg.norm(wn,ord=2))
...
def pseudocont2(omega, v, w, mat, argsdict, mat2=None, mat3=None, M=None, dir=None):
    omegans=[omega]
    vns=[v.copy()]
    wns=[w.copy()]
    vni=v.copy()
    ...
    C,lu,piv=makejac(vni,E_n,E_omega.dot(vni),E_mu.dot(vni),dir0)
    ...
    try:
        while (argsdict[argsdict['par']]-argsdict['pari'])/(argsdict['parf']-argsdict['pari'])<=1 and (argsdict[argsdict['par']]-argsdict['pari'])/(argsdict['parf']-argsdict['pari'])>=0 and steps<argsdict['Nsteps'] and np.linalg.norm(omega)<argsdict['lambdamax']:
            ...
            for n in range(argsdict['itmax']):
                ...
                verr=np.linalg.norm(dv,ord=2)/(1+np.linalg.norm(vni,ord=2))
                ...
                # mstp=1-(stp.dot(M*dir0))/(stp.dot(M*stp)**0.5*dir0.dot(M*dir0)**0.5)
                mstp=1-(stp.dot(M*dir0))/(stp.dot(M*stp)**0.5)
                ...
                if verr<argsdict['epsu'] and omegaerr<argsdict['epsl'] and muerr<argsdict['epsl'] and (steps==0 or mstp<argsdict['epstp']):
                    break
                if n>2 and ((verr>2*verrlast and verr>argsdict['epsu']) or (muerr>2*muerrlast and muerr>argsdict['epsl']) or (omegaerr>2*omegaerrlast and omegaerr>argsdict['epsl'])):
                    break
                verrlast=verr
                muerrlast=muerr
            ...
            if verr<argsdict['epsu'] and omegaerr<argsdict['epsl'] and muerr<argsdict['epsl'] and (steps==0 or mstp<argsdict['epstp']):
                ...
                for m in range(10):
                    ...
                    direrr=np.linalg.norm(M*cor,ord=2)/(1+np.linalg.norm(M*dir0,ord=2))
                    if argsdict['verbose']:
                ...
                if argsdict['verbose']:
                    print('deltadir=%.3e stp=(%.3f,%.3f,%.3f) newdir=(%.3f, %.3f, %.3f)'%(deltadir,stp[-3],stp[-2],stp[-1],dir0[2*s],dir0[2*s+1],dir0[2*s+2]))

                if direrr>argsdict['epdir'] or deltadir>argsdict['epdir']:
                ...
                if np.abs(ds)<argsdict['dsmin']:
                    if argsdict['verbose']>0:
                        print('Minimum step!')
                    return omegans,vns,wns,parns,dirs
...
parser.add_argument("--epstp", type=float, required=False, default=1E-1, dest='epstp', help='Step direction tolerance')
...
parser.add_argument("--thu", type=float, required=False, default=1E-6, dest='thu', help='Pseudoarclength weight for eigenvectors.')
...
if __name__=="__main__":
    ...
    omegas,vs,ws,pars,dirs=pseudocont2(omega0,v0,w0,mat,argsdict,dir=dir0)
